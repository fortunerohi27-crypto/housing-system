  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div><label className="label">From</label>
        <select className="input" value={form.from} onChange={set("from")} required>
                ...
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-stone-50/30 dark:bg-stone-950/40">
                    {active.thread.map((msg, i) => (
                      <div key={i} className={`flex ${msg.who === "manager" ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[75%] rounded-3xl px-4 py-3 text-sm shadow-sm ${msg.who === "manager" ? "bg-stone-800 text-white rounded-br-sm" : "bg-white dark:bg-stone-800 text-stone-800 dark:text-stone-100 rounded-bl-sm border border-stone-200 dark:border-stone-700"}`}>
                          {msg.text}
                          <div className={`text-[10px] mt-1 opacity-60 ${msg.who === "manager" ? "text-stone-200" : "text-stone-500"}`}>{msg.t}</div>
                        </div>
                      </div>
                    ))}
                  </div>
