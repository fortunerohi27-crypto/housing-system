...
function PaymentCard({ invoice }) {
  ...
  async function handlePay() {
    setIsPaying(true);
    try {
      await actions.markInvoicePaid({ id: invoice.id, method: "Tenant portal" });
    } catch (err) {
    ...
    <div className="card p-5 border-l-4 border-l-sky-500">
      <div className="flex justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-1">Due {fmtDate(invoice.due)}</p>
        </div>
        <div className={`h-10 w-10 rounded-xl grid place-items-center ${unpaid ? "bg-sky-100 text-sky-600" : "bg-emerald-100 text-emerald-600"}`}>
          {unpaid ? <CreditCard size={20} /> : <CheckCircle2 size={20} />}
        </div>
      </div>
      {unpaid && (
        <button
          className="btn-primary mt-4 w-full justify-center bg-sky-600 hover:bg-sky-700 disabled:opacity-50"
          disabled={isPaying}
          onClick={handlePay}
        >
          {isPaying ? "Processing..." : "Pay rent securely"}
        </button>
      )}
    </div>
  );
...
function Maintenance({ resident }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      await actions.addMaint({
        title: form.title,
        priority: form.priority,
        status: "Open",
        tenantId: resident.tenant.id,
    ...
    <main className="p-4 md:p-6">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">Tell us what needs attention in your home.</p>
        </div>
        <button className="btn-primary bg-sky-600 hover:bg-sky-700" onClick={() => setOpen(true)}>
      ...
      <div className="space-y-3">
        {resident.tickets.length ? resident.tickets.map((ticket) => (
          <div className="card p-5 flex items-center gap-4" key={ticket.id}>
            <div className="h-10 w-10 rounded-xl bg-sky-100 text-sky-600 grid place-items-center">
              <Wrench size={18} />
            </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="New maintenance request">
        <form className="space-y-4" onSubmit={submit}>
          <div>
            ...
            <textarea
              className="input rounded-2xl min-h-28"
              value={form.title}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="btn-primary bg-sky-600 hover:bg-sky-700 disabled:opacity-50" disabled={isSaving}>
  ...
  async function send(e) {
    ...
    try {
      ...
      await actions.addMessage({
        from: resident.tenant.name,
        tenantId: resident.tenant.id,
        subject: "Message from tenant portal",
        preview: text.trim(),
        time: now,
        unread: true,
        thread: [{ from: resident.tenant.name, text: text.trim(), time: now }]
      });
      setText("");
    } catch (err) {
  ...
  return (
    <main className="p-4 md:p-6 max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-stone-500 mt-1">Contact your property manager. We’ll keep your conversation here.</p>
      </div>
      <div className="card overflow-hidden">
        <div className="p-5 border-b border-stone-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-600">Typically responds within one business day</p>
          </div>
        </div>
        <div className="p-5 space-y-3 min-h-64 bg-stone-50/60">
          {resident.messages.length ? resident.messages.flatMap((message) =>
            message.thread?.length ? message.thread : [{ from: message.from, text: message.preview, time: message.time }]
          ).map((m, index) => (
            <div key={index} className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${m.from === resident.tenant.name ? "ml-auto bg-sky-600 text-white" : "bg-white border border-stone-100 text-stone-700"}`}>
              <p>{m.text}</p>
              <p className={`text-[10px] mt-1 ${m.from === resident.tenant.name ? "text-sky-100" : "text-stone-400"}`}>{m.from} · {fmtDate(m.time)}</p>
            </div>
        ...
        <form onSubmit={send} className="p-4 flex gap-3 border-t border-stone-100">
          ...
          <button
            ...
            title="Send message"
          >
            {isSending ? "Sending..." : <Send size={16} />}
          </button>
        </form>
      </div>
    </main>
